...
class ConfigFileDisk:
    """Сохраняет конфигурационный файл на диск, запрашивая в консоли у
    пользователя имя для файла и проверяя возможность сохранения файла
    под этим именем.
    ...
    def get_file_name(self, filename):
        """Получает подходящее имя для записи файла конфигурации.

        Сначала предлагается заданное имя для файла конфигурации. Если
        это имя файла занято другим файлом, то пользователю
        предлагается перезаписать этот файл. Если пользователь
        соглашается перезаписать этот файл, то предложенное имя файла
        возвращается в качестве подходящего. Если пользователь не
        соглашается перезаписать этот файл, то у пользователя
        запрашивается новое имя для записи файла конфигурации.

        Аргументы:

        filename -- первоначальное предлагаемое имя для файла

        Возвращается:

        Подходящее имя для записи файла конфигурации, если подходящее
        имя найдено.

        None - если пользователь отказался перезаписывать существующий
        файл и новое имя не было задано пользователем или было задано
        пользователем, но не подошло для записи файла конфигурации,
        так как такой файл также существует, а его перезапись не была
        разрешена
    ...
    def save(self, filename, text):
        """Сохраняет в файл конфигурации текст конфигурации.

        Аргументы:

        filename -- имя файла для сохранения конфигурации
        text -- текст конфигурации

        Возвращается:

        True - если файл сохранился
        False - если файл не сохранился
...
class ConfigFileDiskDialog:
    """Проводит диалоги с пользователем в консоли про конфигурационный
    файл на диске, возвращая результаты, введённые пользователем.
    ...
    def ask_for_rewrite(self, filename):
        """Запрашивает у пользователя разрешение на перезапись файла.

        У пользователя запрашивается разрешение на перезапись
        файла. Если пользователь вводит y, то перезапись файла
        разрешается. Если пользователь вводит что-то другое, то
        перезапись файла запрещается.

        Аргументы:

        filename -- имя файла для перезаписи

        Возвращается:

        Имя файла для перезаписи

        None - если перезапись файла запрещена
    ...
    def ask_for_another_filename(self):
        """Запрашивает у пользователя новое имя для записи файла.

        У пользователя запрашивается новое имя для записи файла. Если
        вводится пустая строка, то ввод прекращается и имя для записи
        файла считается незаданным. Если при вводе имени для записи
        файла обнаруживается, что файл с таким именем уже существует,
        то ввод повторяется. Ввод повторяется три раза. Если за три
        раза новое имя для записи файла не задано, то новое имя для
        записи файла считается незаданным.

        Возвращается:

        Новое имя для записи файла

        None - если новое имя для записи файла не было введено или
        введённое новое имя файла не подходит для записи файла, так
        как файл с таким именем существует
